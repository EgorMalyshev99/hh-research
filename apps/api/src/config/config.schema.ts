...
export const configSchema = z.object({
  ...
  HH_OAUTH_AUTHORIZE_URL: z.string().url().default('https://hh.ru/oauth/authorize'),
  HH_OAUTH_TOKEN_URL: z.string().url().default('https://api.hh.ru/token'),
  HH_OAUTH_CLIENT_ID: z.string().min(1, 'HH_OAUTH_CLIENT_ID обязателен'),
  HH_OAUTH_CLIENT_SECRET: z.string().min(1).optional(),
  HH_OAUTH_REDIRECT_URI: z.string().url('HH_OAUTH_REDIRECT_URI должен быть корректным URL'),
